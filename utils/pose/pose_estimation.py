...
def get_query_images(query_txt, img_txt, pgt_type=None):
    
    if pgt_type == 'orig_test_BA':
        query_img_names = get_query_image_names_new(query_txt)
    else:
        query_img_names = get_query_image_names(query_txt)
    
    # print('query_img_names:', query_img_names)
    # print('img_txt:', img_txt)

    # Find query ID corresponding to the name
    query_img_id = []
    print('custom file names appedned!')
    for k, v in img_txt.items():
        ...
        # if dataset == '12scenes':
        #     query_name = v.name.split('/')[1]
        #     if query_name in query_img_names:
        #         query_img_id.append(v.id)
