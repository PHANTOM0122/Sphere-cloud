...
def drawlines_ray_clouds_single(pts3d, pts_ids, center_point):
    '''
    pts3d: 3D points
    ...
    return pts3d, lines, ind_to_id, id_to_ind

def drawlines_ray_clouds_multi(pts3d, pts_ids, center_points):
    '''
    pts3d: 3D points
    pts_ids: 3D point ids
    center_points: center points
...
def drawlines_ray_clouds_voxelize(pts3d, pts_ids, center_points, voxel_size):
    ...
    for i, pt in enumerate(pts3d):
        ...
        center_point_idx = voxel_3d_to_1d(np.asarray(voxel_grid.get_voxel(pt))) % len(center_points)
...
def drawlines_ray_clouds_voxelize_multi(pts3d, pts_ids, anchor_points, voxel_size):
    
    num = len(pts3d)
    num_centers = int(len(anchor_points))
    lines = []
    anc_pts = []
    lines_anchor_indices = []

    # Voxelize
    pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(pts3d))
    voxel_grid = o3d.geometry.VoxelGrid.create_from_point_cloud(pcd, voxel_size=voxel_size)
    print('Voxel number:', voxel_grid)
    print('Voxel grid ids:', )

    # 복셀 그리드의 너비, 높이, 깊이 계산
    voxel_grid_size = voxel_grid.get_max_bound() - voxel_grid.get_min_bound()
    width, height, depth = np.ceil(voxel_grid_size / voxel_size).astype(int)
    width += 1
    height += 1
    depth += 1
    max_value = np.max([width, height, depth])
    print('max value from widht, height, depth:', max_value)

    # 3D 인덱스를 1D 인덱스로 변환
    def to_linear_index(x, y, z):
        # return x * (height * depth) + y * depth + z
        # return z * (width * height) + y * width + x
        return x * (max_value * max_value) + y * max_value + z


    # 해당 값이 있는 배열의 인덱스 찾기
    def find_value_in_arrays(arrays, value):
    ...
    for voxel in voxel_grid.get_voxels():
        ...
        # print('x:', x, 'y:', y, 'z:', z)
        linear_index = to_linear_index(x, y, z)
        linear_indices.append(linear_index)
        
    linear_indices = np.array(linear_indices)    
    np.random.shuffle(linear_indices)
    print('np.max linear_indices:', np.max(linear_indices), 'np.min linear_indices:', np.min(linear_indices))
    
    # splitted_linear_indices = np.array_split(linear_indices, num_centers)
    
    print('len linear_indices:', len(linear_indices), linear_indices.shape)
    print('unique len linear indices:', len(np.unique(linear_indices)))
    assert len(linear_indices) == len(np.unique(linear_indices))

    for i, pt in enumerate(pts3d):
        # anchor points의 갯수 range중 하나 선택
        # anchor_point_idx = find_value_in_arrays(splitted_linear_indices, to_linear_index(voxel_grid.get_voxel(pt)[0], voxel_grid.get_voxel(pt)[1], voxel_grid.get_voxel(pt)[2]))
        anchor_point_idx = to_linear_index(voxel_grid.get_voxel(pt)[0], voxel_grid.get_voxel(pt)[1], voxel_grid.get_voxel(pt)[2]) % num_centers
        
        anchor_point = anchor_points[anchor_point_idx]
        line = np.subtract(anchor_point, pt)
        lines.append(line)
        anc_pts.append(anchor_point)
        lines_anchor_indices.append(anchor_point_idx)

    lines = np.array(lines, dtype=np.float64).reshape(-1, 3)
    lines /= (np.linalg.norm(lines, axis=1, keepdims=True) + 1e-7)

    anc_pts = np.asarray(anc_pts, dtype=np.float64).reshape(-1, 3)
    pair_ind = np.arange(num)
    ind_to_id, id_to_ind = id_ind_connect([pair_ind], pts_ids)
    
    lines_anchor_indices = np.array(lines_anchor_indices)
    for i in range(num_centers):
        print(f'{i}th voxel assigned points num:', len(np.where(lines_anchor_indices==i)[0]))

    return pts3d, lines, ind_to_id, id_to_ind, anc_pts, lines_anchor_indices
...
def drawlines_permutation(pts,ids):
    num = len(pts)
    np.random.seed(91)
    lines = np.random.randn(num,3)
    lines /= np.linalg.norm(lines, axis=1, keepdims=True)+1e-7
