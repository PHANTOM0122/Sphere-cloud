...
std::pair<CameraPose, py::dict> estimate_generalized_relative_pose_wrapper(
    ...
    std::cout << "P5+1R solver (24.04.18 update)" << std::endl;
...
std::pair<CameraPose, py::dict> estimate_sphere_pose_wrapper(
    const std::vector<Eigen::Vector3d> points3D, 
    ...
    std::cout << "Sphere cloud pose estimation." << std::endl;

    std::vector<Camera> cameras;
    for (const py::dict &camera_dict : cameras_dict) {
        cameras.push_back(camera_from_dict(camera_dict));
    }
    ...
    RansacStats stats = estimate_sphere_pose(points3D, lines3D, points2D, points3D_rgbd, cameras, lambda1, lambda2, ransac_opt, bundle_opt, &pose, &inlier_mask, runRefine);
...
PYBIND11_MODULE(poselib, m) {
    ...
    m.def("estimate_sphere_pose", &poselib::estimate_sphere_pose_wrapper,
          py::arg("points3D"), py::arg("lines3D"), py::arg("points2D"), py::arg("points3D_rgbd"), py::arg("camera_dict"), 
          py::arg("lambda1"), py::arg("lambda2"), py::arg("ransac_opt") = py::dict(), py::arg("bundle_opt") = py::dict(), py::arg("runRefine"),
          "Virtutal absolute pose estimation with non-linear refinement. Note: This is modified implemenation for sphere cloud and depth image.");   
